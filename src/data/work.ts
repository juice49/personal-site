interface Work {
  name?: string,
  description?: string
}

const work: Work[] = [
  {
    description: 'Implemented Incremental Static Regeneration for a Next.js site.'
  },
  {
    description: 'Built a GraphQL API layer over an old and inconsistent REST-ish API.'
  },
  {
    description: 'Built a CSS-in-JS helper library.'
  },
  {
    description: 'Built a headless site builder with WordPress and Next.js.'
  },
  {
    description: 'Setup Next.js preview mode with WordPress.'
  },
  {
    description: 'Created a Connect 4 game in TypeScript and React.'
  }
]

export default work
