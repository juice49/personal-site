...
{/* prettier-ignore */}
<Note>The flow of props in a statically rendered Next.js page.</Note>
...
Shu, a developer at Vercel, shows us that this can be achieved with Next.js. The [Next-CMS project](https://github.com/shuding/next-cms) provides a `<CMS />{:tsx}` component that can be used to fetch and statically insert data from WordPress in any component.
...
The `<CMS />{:tsx}` component accepts an `endpoint` prop to define its data source, and the fetched data is passed into a child render function. The assigned endpoint is automatically lifted up to the page's `getStaticProps` function and fetched when the page is pre-rendered.
...
[A function](https://github.com/shuding/next-cms/blob/master/lib/get-cms-static-props.js) is provided that gathers and fetches the endpoints assigned to nested `<CMS />{:tsx}` components. It's called from inside the `getStaticProps` function, and uses ReactDOM's `renderToStaticMarkup` to perform an extra page render _prior_ to Next.js' pre-render.

During this extra render, the endpoints of all nested `<CMS />{:tsx}` components are [pushed to a global array](https://github.com/shuding/next-cms/blob/f39cddc58b6b400d9dfa5fe14714c52b72e5bf7c/lib/get-cms-static-props.js#L10). They are added to a [`Set`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Set)—which is a convenient way to deduplicate them—and [then resolved in parallel using `fetch`](https://github.com/shuding/next-cms/blob/f39cddc58b6b400d9dfa5fe14714c52b72e5bf7c/lib/get-cms-static-props.js#L23).
...
{/* prettier-ignore */}
<Note>Gathering static props before pre-render. Each CMS component pushes its assigned endpoint to a global array. Once the required endpoints have been lifted up to the getStaticProps function, they are fetched in parallel. The fetched data is passed into the page props when Next.js pre-renders the page.</Note>

The fetched data is [added to an object](https://github.com/shuding/next-cms/blob/f39cddc58b6b400d9dfa5fe14714c52b72e5bf7c/lib/get-cms-static-props.js#L32), where its key is the requested endpoint. This object is [passed into the page props](https://github.com/shuding/next-cms/blob/f39cddc58b6b400d9dfa5fe14714c52b72e5bf7c/pages/index.js#L25). Because the key is the requested endpoint, it can be mapped back to the relevant `<CMS />{:tsx}` component when the page is rendered.

### 2. React Context

The `withCMSPage` _higher order component_ wraps the page in [a React Context](https://github.com/shuding/next-cms/blob/f39cddc58b6b400d9dfa5fe14714c52b72e5bf7c/lib/with-cms-page.js#L11). The Context is initialised using the fetched data from the page props. `<CMS />{:tsx}` components nested at any level can then read the data returned from their assigned endpoint.
...
{/* prettier-ignore */}
<Note>The fetched data is injected into the CMS components using React Context.</Note>

### 3. The CMS component

[The `<CMS />{:tsx}` component](https://github.com/shuding/next-cms/blob/f39cddc58b6b400d9dfa5fe14714c52b72e5bf7c/lib/cms.js#L5) runs in two different modes.
...
During a normal render, the `<CMS />{:tsx}` component reads the fetched data from Context, and then passes it down to its [child render function](https://github.com/shuding/next-cms/blob/f39cddc58b6b400d9dfa5fe14714c52b72e5bf7c/lib/cms.js#L11).

---

There's a few connected parts to think about here. The important thing is that it's relatively simple for users to set up. The [`withCMSPage` and `getCMSStaticProps`](https://github.com/shuding/next-cms/blob/master/pages/index.js) function are all we need to add to start using the `<CMS />{:tsx}` component.
